            ...
            scannerHome = tool 'sonar' // the name you have given the Sonar Scanner (Global Tool Configuration)
                ...
                steps {
                    withSonarQubeEnv('sonar') {
                        sh ''' $scannerHome/bin/sonar-scanner -Dsonar.projectName=goat-v1 -Dsonar.url=http://172.28.208.1:9000/ \
                        -Dsonar.login=sqp_b2261d6bb9e293ec434456cbf0e9ffa107d6c81a  -Dsonar.projectKey=goat-v1 -Dsonar.java.binaries=. '''
                    }
                }
            }
